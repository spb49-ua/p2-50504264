...
function prepArgsBin ()
{
  ...
  if test "$1" == "05_saveData" ; then
    VAL="" # desactivar valgrind para esta prueba
  elif test "$1" == "06_argument-i" ; then
    args="-i pruebas/fileImportFromCSV.csv"
  elif test "$1" == "07_argument_error" ; then
    args="-l "
  fi
}
  ...
  if test -s $tempfile; then
     ...
     if [ $? -ne 0 ]; then
       diff -EwB $OBT/${bn}.salida-obtenida $SAL/${bn}.salida-esperada 2>&1
        ...
        if test "$bn" == "03_exportToCSV"; then
 	        $comparefiles $OBT/fileExportToCSV.csv $SAL/fileExportToCSV.csv
	        if [ $? -ne 0 ]; then
		        diff -EwB $OBT/fileExportToCSV.csv $SAL/fileExportToCSV.csv 2>&1
	        else
 	          echo "OK"
	          let pok++
	        fi           
	elif test "$bn" == "05_saveData"; then
 	        $VAL $cmpBinFiles $OBT/fileSaveData.bin $SAL/fileSaveData.bin 2>&1
		if [ $? -eq 0 ]; then
 		          echo "OK"
		          let pok++
 		fi
