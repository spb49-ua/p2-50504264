...
Hero attack_defense(Hero player){
  ...
  do{
    ...
    cin>>attack>>aux>>defense;
    if(attack<=0 || defense<=0 || (attack+defense)!=100){
      cout<<"ERROR: wrong distribution"<<endl;
      error=true;
    }
  }while(error);
  
  player.features.attack=attack*KPOINTS/100;
  player.features.defense=defense*KPOINTS/100;
...
Enemy enemyStats(Enemy enemigo){
  ...
  switch(enemigo.name){
    ...
    cout<<endl
        <<"Attack: "<<enemigo.features.attack<<endl
        <<"Defense: "<<enemigo.features.defense<<endl
        <<"Health points: "<<enemigo.features.hp<<endl;
...
void heroToEnemy(Hero &player,Enemy &enemigo,int extra){
  int dice1,dice2;
  dice1=rollDice()*5*extra;
  dice2=rollDice()*5;
  
  cout<<"[Hero -> Enemy]"<<endl
      <<"Attack: "<<player.features.attack<<" + "<<dice1<<endl
      <<"Defense: "<<enemigo.features.defense<<" + "<<dice2<<endl
      <<"Hit points: ";
  
  if((player.features.attack+dice1-(enemigo.features.defense+dice2))>0){
    cout<<player.features.attack+dice1-(enemigo.features.defense+dice2);
    enemigo.features.hp=enemigo.features.hp-(player.features.attack+dice1-(enemigo.features.defense+dice2));
  }
  else
    cout<<"0";
...
void enemyToHero(Hero &player,Enemy &enemigo){
  ...
  cout<<"[Enemy -> Hero]"<<endl
      <<"Attack: "<<enemigo.features.attack<<" + "<<dice1<<endl
      <<"Defense: "<<player.features.defense<<" + "<<dice2<<endl
      <<"Hit points: ";
  
  if((enemigo.features.attack+dice1-(player.features.defense+dice2))>0){
    cout<<enemigo.features.attack+dice1-(player.features.defense+dice2);
    player.features.hp=player.features.hp-(enemigo.features.attack+dice1-(player.features.defense+dice2));
  }
  else
  	cout<<"0";
...
void report(const Hero &player){
  ...
  cout<<"[Report]"<<endl
      ...
      <<"Attack: "<<player.features.attack<<endl
      <<"Defense: "<<player.features.defense<<endl
      <<"Health points: "<<player.features.hp<<endl
      <<"Special: ";
  ...
  if(player.special)
   ...
   cout<<endl
       <<"Runaways: "<<player.runaways<<endl
       <<"Exp: "<<player.exp<<endl
       <<"Enemies killed:"<<endl
       <<"- Axolotl: "<<player.kills[0]<<endl
       <<"- Troll: "<<player.kills[1]<<endl
       <<"- Orc: "<<player.kills[2]<<endl
       <<"- Hellhound: "<<player.kills[3]<<endl
       <<"- Dragon: "<<player.kills[4]<<endl
       <<"- Total: ";
